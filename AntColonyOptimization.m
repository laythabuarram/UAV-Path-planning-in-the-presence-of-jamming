% Professional Ant Colony Optimization (ACO) for Path Planning
% Modern, professional visualization with improved color scheme
% Both subplots have equal height (same y-axis scaling)
...
%% ================= FIGURE SETUP WITH EQUAL HEIGHT =================
figure('Position',[100 100 1200 600],'Color',[0.97 0.97 0.97]);

% -------- Left subplot (grid + animation)
ax1 = subplot(1,2,1);

% Create custom colormap for grid: [free cells, obstacles]
grid_cmap = [0.95 0.95 0.95;  % Light grey for free cells
             0.4  0.4  0.4];   % Dark grey for obstacles
imagesc(grid);
colormap(ax1, grid_cmap);
caxis([0 1]);
axis equal tight;
set(ax1,'YDir','normal','Color',[0.98 0.98 0.98]);  % Very light background
xlabel('Column'); ylabel('Row');
title('ACO Path Planning - Professional Visualization');

% Set equal height by matching y-axis limits to grid dimensions
ylim(ax1, [0.5 grid_size(1)+0.5]);  % Match grid height
xlim(ax1, [0.5 grid_size(2)+0.5]);  % Keep grid width

hold on;

% Start & goal with Option 1 colors
...
% Create custom sequential blue colormap for pheromone
pheromone_cmap = [linspace(0.95,0.2,64)', ...    % R: light to dark blue
                  linspace(0.95,0.5,64)', ...    % G: light to medium
                  linspace(1.0,0.8,64)'];        % B: very light to light
...
% Add start/goal labels for clarity
...
% Set y-axis to match grid height (same number of units)
% Since grid is 20x20, maximum Manhattan distance is about 38-40
% We'll set initial y-limits to match grid dimensions
ylim([0 grid_size(1)*2]);  % Same height scale as left plot

% Add colorbar for pheromone levels
cb = colorbar(ax1);
cb.Label.String = 'Pheromone Intensity';
cb.Label.FontSize = 10;
cb.Color = [0.2 0.2 0.2]; % Dark text for colorbar

% Adjust subplot positions to ensure equal visual height
...
for iter = 1:n_iterations
    ...
    % Update convergence plot y-axis while maintaining equal height concept
    finite_vals = history_best_lengths(isfinite(history_best_lengths));
    if ~isempty(finite_vals)
        current_min = min(finite_vals);
        current_max = max(finite_vals);
        
        % Ensure y-axis has similar scale to left plot
        if current_max > current_min
            % Add some padding but keep similar scale to grid height
    ...
    % Update convergence plot title with current best
...
% Add final best path length to convergence plot
...
% Add grid lines to match the grid visualization style
grid(ax2, 'on');
grid(ax2, 'minor');
